...
export const toMessage = (event: MessageEvent): RebuildMessage | ErrorMessage | ConnectedMessage | null => {
    try {
        const parsed = JSON.parse(event.data as string);
        if (isRebuildMessage(parsed) || isConnectedMessage(parsed) || isErrorMessage(parsed)) {
            return parsed;
        } else {
            return null;
        }
